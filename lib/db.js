...
const REPOSTS_FILE = path.join(__dirname, '..', 'data', 'reposts.json');
...
function deletePost(id) {
  ...
  const reposts = readJson(REPOSTS_FILE).filter((r) => r.postId !== String(id));
  writeJson(REPOSTS_FILE, reposts);
}
...
function createRepost(postId) {
  const post = getPostById(postId);
  if (!post) return null;
  const reposts = readJson(REPOSTS_FILE);
  const repost = {
    id: nextId(reposts),
...
function getRepostCounts() {
  const reposts = readJson(REPOSTS_FILE);
  return reposts.reduce((counts, r) => {
...
function getFeedPosts() {
  ...
  const items = posts.map((p) => ({ post: p, isRepost: false, sortDate: p.date }));
  reposts.forEach((r) => {
    const post = postsById.get(r.postId);
    if (post) {
      items.push({ post, isRepost: true, repostDate: r.date, sortDate: r.date });
    }
  });
